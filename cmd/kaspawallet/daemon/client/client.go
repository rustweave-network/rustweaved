...
import (
	"context"
	"github.com/rustweave-network/rustweaved/cmd/kaspawallet/daemon/server"
	"time"
...
func Connect(address string) (pb.KaspawalletdClient, func(), error) {
	...
	if err != nil {
		if errors.Is(err, context.DeadlineExceeded) {
			return nil, nil, errors.New("rustweavewallet daemon is not running, start it with `kaspawallet start-daemon`")
		}
		return nil, nil, err
	}
