...
func main() {
	...
	if !cfg.AllowConnectionToDifferentVersions {
		rustweavedMessage, err := client.Post(&protowire.RustweavedMessage{Payload: &protowire.RustweavedMessage_GetInfoRequest{GetInfoRequest: &protowire.GetInfoRequestMessage{}}})
		if err != nil {
			printErrorAndExit(fmt.Sprintf("Cannot post GetInfo message: %s", err))
		}

		localVersion := version.Version()
		remoteVersion := rustweavedMessage.GetGetInfoResponse().ServerVersion
...
func prettifyResponse(response string) string {
	rustweavedMessage := &protowire.RustweavedMessage{}
	err := protojson.Unmarshal([]byte(response), rustweavedMessage)
	if err != nil {
	...
	return marshalOptions.Format(rustweavedMessage)
}
