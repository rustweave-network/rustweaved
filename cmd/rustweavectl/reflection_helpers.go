...
// 2. A wrapper that implements isRustweavedMessage_Payload, having a single field pointing to the concrete command
//    bearing the name of the command with `RustweavedMessage_` prefix and `Request` suffix
...
// generateRustweavedMessage generates a wrapped RustweavedMessage with the given `commandValue`
func generateRustweavedMessage(commandValue reflect.Value, commandDesc *commandDescription) (*protowire.RustweavedMessage, error) {
	commandWrapper := reflect.New(commandDesc.typeof)
	unwrapCommandValue(commandWrapper).Set(commandValue)

	rustweavedMessage := reflect.New(reflect.TypeOf(protowire.RustweavedMessage{}))
	rustweavedMessage.Elem().FieldByName("Payload").Set(commandWrapper)
	return rustweavedMessage.Interface().(*protowire.RustweavedMessage), nil
}
