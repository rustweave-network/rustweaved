...
func TestVirtualSelectedParentChain(t *testing.T) {
	// Setup a couple of rustweaved instances
	rustweaved1, rustweaved2, _, teardown := standardSetup(t)
	defer teardown()
	...
	err := rustweaved1.rpcClient.RegisterForVirtualSelectedParentChainChangedNotifications(true,
	...
	// In rustweaved2, mine a different chain of `blockAmountToMine` + 1
	// blocks over the genesis
	var chain2Tip *externalapi.DomainBlock
	for i := 0; i < blockAmountToMine+1; i++ {
		chain2Tip = mineNextBlock(t, rustweaved2)
	}

	// Connect the two rustweaveds. This should trigger sync
	// between the two nodes
	connect(t, rustweaved1, rustweaved2)
	...
	// the first chain
	virtualSelectedParentChainFromChain1Tip, err := rustweaved1.rpcClient.GetVirtualSelectedParentChainFromBlock(
