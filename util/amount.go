...
// other than the base unit of a rustweave. The value of the AmountUnit
// is the exponent component of the decadic multiple to convert from
// an amount in rustweave to an amount counted in units.
type AmountUnit int
...
// Amount represents the base rustweave monetary unit (colloquially referred
// to as a `Sompi'). A single Amount is equal to 1e-8 of a rustweave.
type Amount uint64
...
// some value in rustweave. NewAmount errors if f is NaN or +-Infinity, but
...
func NewAmount(f float64) (Amount, error) {
	...
	case math.IsInf(f, 1):
		fallthrough
	case math.IsInf(f, -1):
		return 0, errors.New("invalid rustweave amount")
	}

	return round(f * constants.SompiPerRustweave), nil
}

// ToUnit converts a monetary amount counted in rustweave base units to a
// floating point value representing an amount of rustweave.
...
// useful for services that build on top of rustweave (for example, calculating
