...
// Outpoint defines a rustweave data type that is used to track previous
// transaction outputs.
...
// NewOutpoint returns a new rustweave transaction outpoint point with the
...
// TxIn defines a rustweave transaction input.
type TxIn struct {
...
// NewTxIn returns a new rustweave transaction input with the provided
...
// TxOut defines a rustweave transaction output.
type TxOut struct {
	Value        uint64
	ScriptPubKey *externalapi.ScriptPublicKey
}

// NewTxOut returns a new rustweave transaction output with the provided
...
// MsgTx implements the Message interface and represents a rustweave tx message.
