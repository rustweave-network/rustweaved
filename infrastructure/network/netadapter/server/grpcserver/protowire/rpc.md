...
Clients are expected to build RequestMessages and wrap them in RustweavedMessage. (see messages.proto)

Having received a RequestMessage, (wrapped in a RustweavedMessage) the RPC server will respond with a
ResponseMessage (likewise wrapped in a RustweavedMessage) respective to the original RequestMessage.
...
GetCurrentNetworkRequestMessage requests the network rustweaved is currently running against.
...
| Field | Type | Label | Description |
| ----- | ---- | ----- | ----------- |
| payAddress | [string](#string) |  | Which rustweave address should the coinbase block reward transaction pay into |
...
| ----- | ---- | ----- | ----------- |
| block | [RpcBlock](#protowire.RpcBlock) |  |  |
| isSynced | [bool](#bool) |  | Whether rustweaved thinks that it&#39;s synced. Callers are discouraged (but not forbidden) from solving blocks when rustweaved is not synced. That is because when rustweaved isn&#39;t in sync with the rest of the network there&#39;s a high chance the block will never be accepted, thus the solving effort would have been wasted. |
| error | [RPCError](#protowire.RPCError) |  |  |
...
GetPeerAddressesRequestMessage requests the list of known rustweaved addresses in the
...
currently connected to this rustweaved.
...
| isOutbound | [bool](#bool) |  | Whether this rustweaved initiated the connection |
| timeOffset | [int64](#int64) |  |  |
| userAgent | [string](#string) |  |  |
| advertisedProtocolVersion | [uint32](#uint32) |  | The protocol version that this peer claims to support |
| timeConnected | [int64](#int64) |  | The timestamp of when this peer connected to this rustweaved |
...
AddPeerRequestMessage adds a peer to rustweaved&#39;s outgoing connection list.
This will, in most cases, result in rustweaved connecting to said peer.
...
parent chain from some startHash to this rustweaved&#39;s current virtual
...
GetBlockCountRequestMessage requests the current number of blocks in this rustweaved.
...
ShutDownRequestMessage shuts down this rustweaved.
...
This call is only available when this rustweaved was started with `--utxoindex`
...
GetUtxosByAddressesRequestMessage requests all current UTXOs for the given rustweaved addresses
